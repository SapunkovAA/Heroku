...
http.createServer(app).listen(PORT, console.log("Server started successfully at port" + PORT)); //получаем модуль http, который необходим для создания сервера. Это встроенный модуль, и для его загрузки необходимо применить функцию require():
